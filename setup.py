...
if not ISRELEASED:
    ...
    if pipe is None or pipe.returncode != 0:
        ...
        if os.path.exists("recursive_diff/version.py"):
            warnings.warn(
                "WARNING: Couldn't get git revision,"
                " using existing recursive_diff/version.py"
            )
            write_version = False
        else:
            warnings.warn(
...
"""
    if not filename:
        filename = os.path.join(
            os.path.dirname(__file__), "recursive_diff", "version.py"
        )
...
setup(
    ...
    entry_points={"console_scripts": ["ncdiff = recursive_diff.ncdiff:main"]},
